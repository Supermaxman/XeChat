    ...
    protected void run(Player player, String[] args) {
        if (args.length >= 2) {
            ...
            if (player.getServer().getPlayerExact(args[0]) != null) {
                ...
                player.sendMessage(ChatColor.AQUA + "[XeChat]: Now Whispering " + XeChat.chat.getPlayerPrefix(player.getServer().getPlayerExact(args[0])).replaceAll("&", "�") + player.getServer().getPlayerExact(args[0]).getName() + ChatColor.AQUA + ".");
            } else {
                player.sendMessage(ChatColor.RED + "[XeChat]: ERROR, The Player " + args[0] + " Is Not Online Or Does Not Exist.");
            }
        } else {
